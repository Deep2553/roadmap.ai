...
store_credential() { # id username secret_file description
  ...
  if [ "$exists" = "200" ]; then
    http=$(api -o /dev/null -w '%{http_code}' -X POST -H "$CRUMB_FIELD: $CRUMB" \
      --data-binary "@$payload" \
      "$JENKINS_URL/credentials/store/system/domain/_/credential/$id/updateSubmit")
    echo "   $id: updated (HTTP $http)"
  else
    http=$(api -o /dev/null -w '%{http_code}' -X POST -H "$CRUMB_FIELD: $CRUMB" \
      --data-binary "@$payload" \
      "$JENKINS_URL/credentials/store/system/domain/_/createCredentials")
    echo "   $id: created (HTTP $http)"
  fi
}
...
if [ "$job_exists" = "200" ]; then
  ...
  echo "   job updated (HTTP $http)"
else
  http=$(api -o /dev/null -w '%{http_code}' -X POST -H "$CRUMB_FIELD: $CRUMB" \
    -H 'Content-Type: application/xml' --data-binary "@$JOB_XML" \
    "$JENKINS_URL/createItem?name=$JOB_NAME")
  echo "   job created (HTTP $http)"
fi
